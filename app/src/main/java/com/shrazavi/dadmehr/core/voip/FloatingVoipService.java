...
import com.shrazavi.dadmehr.BuildConfig;
import com.shrazavi.dadmehr.R;
import com.shrazavi.dadmehr.core.CallSession;
import com.shrazavi.dadmehr.core.CallSession.CallSessionCallback;
import com.shrazavi.dadmehr.core.EnumType.CallEndReason;
import com.shrazavi.dadmehr.core.EnumType.CallState;
...
public class FloatingVoipService extends Service {
    ...
    public CountDownTimer cdtfloat;
    private long timer = 0, timerfloating = 0;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        resumeActivityIntent.putExtra(CallSingleActivity.EXTRA_TARGETID, intent.getStringExtra(CallSingleActivity.EXTRA_TARGETID));
        resumeActivityIntent.putExtra(CallSingleActivity.EXTRA_TARGETUSER, intent.getStringExtra(CallSingleActivity.EXTRA_TARGETUSER));
        resumeActivityIntent.putExtra(CallSingleActivity.EXTRA_ROOM, intent.getStringExtra(CallSingleActivity.EXTRA_ROOM));
        resumeActivityIntent.putExtra(CallSingleActivity.EXTRA_CALLERID, intent.getStringExtra(CallSingleActivity.EXTRA_CALLERID));
        resumeActivityIntent.putExtra(CallSingleActivity.EXTRA_CALLERUSER, intent.getStringExtra(CallSingleActivity.EXTRA_CALLERUSER));
        resumeActivityIntent.putExtra(CallSingleActivity.EXTRA_TIMER, intent.getStringExtra(CallSingleActivity.EXTRA_TIMER));
        resumeActivityIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//        timer = Long.parseLong(intent.getStringExtra(CallSingleActivity.EXTRA_TIMER));
    ...
    private void showFloatingWindow() {
        ...
        Log.e("is audio only=", session.isAudioOnly() + "");
...
//        cdtfloat = new CountDownTimer(timer, 1000) {
//            public void onTick(long millisUntilFinished) {
//                timerfloating = millisUntilFinished;
//            }
//
...
//        }.start();
    ...
    private ViewGroup getFloatingView() {
        ...
        Log.e("getFloatingView", "" + session.isAudioOnly());
        if (session.isAudioOnly()) {
            if (audioView == null) {
