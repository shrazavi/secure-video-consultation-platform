...
import com.shrazavi.dadmehr.core.util.OSUtils;
...
public class FragmentVideo extends SingleCallFragment implements View.OnClickListener {
    ...
    private ImageView switchCaptureImageView;
    private ImageView MuteImageView;
    private ImageView switchSpeakerImageView;
    ...
    private Swipe_Button_View start, stop;
...
//        Log.e("session",session.getState()+"");

//            session.joinHome("60e57dcab921c43ac41cf1ab");
    ...
    public void initView(View view) {
        ...
        switchCaptureImageView = view.findViewById(R.id.switchCaptureImageView);
        MuteImageView = view.findViewById(R.id.switchMuteImageView);
        switchSpeakerImageView = view.findViewById(R.id.switchSpeakerImageView);
        start = view.findViewById(R.id.acceptvideocall);
        stop = view.findViewById(R.id.rejectvideocall);
        ...
        switchCaptureImageView.setOnClickListener(this);
        switchSpeakerImageView.setOnClickListener(this);
...
//                startRefreshTime();
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.switchCaptureImageView) {
            if (session != null && session.getState() != CallState.Idle) {
                if (session.toggleCapture(!micEnabled)) {
                    micEnabled = !micEnabled;
                }
                switchCaptureImageView.setSelected(micEnabled);
            }
        }
        if (id == R.id.switchSpeakerImageView) {
            if (session != null && session.getState() != CallState.Idle) {
                if (session.toggleSpeaker(!isSpeakerOn)) {
                    isSpeakerOn = !isSpeakerOn;
                }
                switchSpeakerImageView.setSelected(isSpeakerOn);
            }
        }
        if (id == R.id.switchMuteImageView) {
            if (session != null && session.getState() != CallState.Idle) {
