...
public class Peer implements SdpObserver, PeerConnection.Observer {
    ...
    public Peer(PeerConnectionFactory factory, List<PeerConnection.IceServer> list, String userId, IPeerEvent event, Boolean isaudio) {
        mFactory = factory;
        mIceLis = list;
        mEvent = event;
        mUserId = userId;
        ...
        if (newState == PeerConnection.IceConnectionState.DISCONNECTED || newState == PeerConnection.IceConnectionState.FAILED) {
            new java.util.Timer().schedule(
                    new java.util.TimerTask() {
                        ...
                        public void run() {
                            // your code here
                            Log.e(TAG, "onIceConnectionChange:10 " + newState);
                            if (newState == PeerConnection.IceConnectionState.COMPLETED || newState == PeerConnection.IceConnectionState.CONNECTED || newState == PeerConnection.IceConnectionState.CHECKING) {
