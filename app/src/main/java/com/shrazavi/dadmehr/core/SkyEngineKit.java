package com.shrazavi.dadmehr.core;
...
import static com.shrazavi.dadmehr.core.base.BasicActivity.preferences;
...
public class SkyEngineKit {
    ...
    // 拨打电话
    public boolean startOutCall(Context context, final String room,final String callerId, final String targetId,final String targetUser,
                                final boolean audioOnly,String timer) {
        // 未初始化
        if (avEngineKit == null) {
            Log.e(TAG, "startOutCall error,please init first");
            return false;
        ...
        mCurrentCallSession.setTargetUser(targetUser);
        mCurrentCallSession.setTimer(timer);
        mCurrentCallSession.setIsComing(false);
        mCurrentCallSession.setCallState(EnumType.CallState.Outgoing);
        // 创建房间
        mCurrentCallSession.createHome(room,callerId, 2);
    ...
    // 接听电话
    public boolean startInCall(Context context, final String room, final String targetId, final String targetUser,
                               final boolean audioOnly,String timer) {
        if (avEngineKit == null) {
    ...
    public void endCall() {
        ...
        editor.putString("timer", SingleCallFragment.timing);
        editor.putString("rating",SingleCallFragment.username);
        editor.commit();
        SingleCallFragment.cdt.cancel();
        if (mCurrentCallSession != null) {
            // 停止响铃
    ...
    public void createAndJoinRoom(Context context, String room,String myId) {
        if (avEngineKit == null) {
            Log.e(TAG, "joinRoom error,init is not set");
            return;
        }
        ...
        mCurrentCallSession = new CallSession(context, room, false, mEvent);
        mCurrentCallSession.setIsComing(false);
        mCurrentCallSession.createHome(room, myId,2);
    }
