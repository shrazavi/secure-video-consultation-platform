...
import com.shrazavi.dadmehr.core.engine.EngineCallback;
import com.shrazavi.dadmehr.core.engine.webrtc.WebRTCEngine;
...
public class CallSession implements EngineCallback {
    ...
    // 创建房间
    public void createHome(String room,String myId, int roomSize) {
        executor.execute(() -> {
            if (mEvent != null) {
                mEvent.createRoom(room,myId, roomSize);
            }
        });
    }
    ...
    public void sendRefuse() {
        executor.execute(() -> {
            if (mEvent != null) {
                // 取消拨出
//                mEvent.sendRefuse(mRoomId, mMyId, EnumType.RefuseType.Hangup.ordinal());
    ...
    void sendBusyRefuse(String room, String targetId) {
        executor.execute(() -> {
            if (mEvent != null) {
                // 取消拨出
//                mEvent.sendRefuse(room, mMyId, EnumType.RefuseType.Busy.ordinal());
    ...
    public void leave() {
        executor.execute(() -> {
            if (mEvent != null) {
                mEvent.sendLeave(mRoomId, mMyId,mTargetId,timer);
            }
        });
        // 释放变量
        release(EnumType.CallEndReason.Hangup);
    ...
    public boolean toggleCapture(boolean enable) {
        return iEngine.togglecapture(enable);
    }
    // 设置扬声器
...
//                    inviteList.add(mMyId);
                    Log.e("invite=", "ejra shod");
                    mEvent.sendInvite(mRoomId,mTargetId,mTargetUser, inviteList, mIsAudioOnly,timer);
                }
            } else {
    ...
    public void onDisConnect(String userId, EnumType.CallEndReason reason) {
        Log.e("disconnect", "reason= " + "call on session");
        executor.execute(() -> {
...
//        });
    ...
    public void disconnected(EnumType.CallEndReason reason) {
        Log.e("disconnect", "reason= " + "callsession");
        handler.post(() -> {
            shouldStopRing();
            release(reason);
        });
    ...
    public void onDisconnected(String userId) {
        //断线了，需要关闭通话界面
        if (sessionCallback != null && sessionCallback.get() != null) {
           Log.e(TAG, "onDisconnected sessionCallback.get() != null ");
